...
void DMA_ClearFlags( DMA_TypeDef * DMAx, uint32_t DMA_flags )
{
  ...
  if ( ( DMA_flags & HIGH_ISR_MASK ) != ( uint32_t )RESET ) {
    /* Set DMAx HIFCR register clear flag bits */
    DMAx->HIFCR = ( uint32_t )( DMA_flags & RESERVED_MASK );
  } else {
    /* Set DMAx LIFCR register clear flag bits */
    DMAx->LIFCR = ( uint32_t )( DMA_flags & RESERVED_MASK );
  }
}
