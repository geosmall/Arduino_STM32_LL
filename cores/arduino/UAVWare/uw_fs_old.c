...
static int temp_block_device_read( const struct lfs_config *c, lfs_block_t block,  lfs_off_t off, void *buffer, lfs_size_t size );
static int temp_block_device_prog( const struct lfs_config *c, lfs_block_t block, lfs_off_t off, const void *buffer, lfs_size_t size );
static int temp_block_device_erase( const struct lfs_config *c, lfs_block_t block );
static int temp_block_device_sync( const struct lfs_config *c );

// Optional statically allocated read buffer. Must be cache_size.
// By default lfs_malloc is used to allocate this buffer.
uint8_t temp_read_buf[ LFS_CACHE_SIZE ];

// Optional statically allocated program buffer. Must be cache_size.
// By default lfs_malloc is used to allocate this buffer.
uint8_t temp_prog_buf[ LFS_CACHE_SIZE ];

// Optional statically allocated lookahead buffer. Must be lookahead_size
// and aligned to a 32-bit boundary. By default lfs_malloc is used to
// allocate this buffer.
uint8_t temp_lookahead_buf[ LFS_LOOKAHEAD_SIZE ] __attribute__ ( ( aligned ( 4 ) ) );

// configuration of the file system is provided by this struct
const struct lfs_config temp_FS_cfg = {
  // block device operations
  .read  = temp_block_device_read,
  .prog  = temp_block_device_prog,
  .erase = temp_block_device_erase,
  .sync  = temp_block_device_sync,
  // chip info
  ...
  .read_buffer = temp_read_buf,
  .prog_buffer = temp_prog_buf,
  .lookahead_buffer = temp_lookahead_buf,
};
...
int UW_fs_old_read_file( const char *srcPath, uint8_t *buf, size_t bufSize )
{
  lfs_file_t fsrc;
  int result;
...
int UW_fs_old_write_file( const char *filePath , const uint8_t *buf, size_t bufSize )
{
  lfs_file_t file;
  int result;
...
int UW_fs_old_format( void )
{
  ...
  result = lfs_format( &_FS_lfs, &temp_FS_cfg );
...
int UW_fs_old_mount( void )
{
  ...
  result = lfs_mount( &_FS_lfs, &temp_FS_cfg );
...
int UW_fs_old_dir( const char *path )
{
  int result;
  lfs_dir_t dir;
...
int UW_fs_old_copy_file( const char *srcPath, const char *dstPath )
{
...
int UW_fs_old_move_file( const char *srcPath, const char *dstPath )
{
  if ( !_FS_isMounted ) {
    UVOS_COM_SendString( UVOS_COM_DEBUG, "ERROR: File system is not mounted.\r\n" );
    return FS_ERR_FAILED;
...
int UW_fs_old_file_size( const char *filePath )
{
  struct lfs_info info;
  int result;
...
int UW_fs_old_remove_file( const char *filePath )
{
  int result;
...
int UW_fs_old_run_benchmark( void )
{
  lfs_file_t file;
  int result;
  uint32_t i;
  ...
  UW_fs_old_remove_file( "./bench.txt" );
  UW_fs_old_remove_file( "./copy.txt" );
  ...
  UW_fs_old_copy_file( ( const char * )"./bench.txt", ( const char * )"./copy.txt" );
...
__attribute__( ( unused ) ) static uint32_t UW_fs_old_print_status( void )
{
...
int UW_fs_old_print_hex_file( const char *filePath )
{
  lfs_file_t file;
