...
/* Global Variables */
VOLINFO UVOS_SDCARD_VolInfo;
uint8_t UVOS_SDCARD_Sector[ SECTOR_SIZE ];
...
int32_t UVOS_SDCARD_Init( uint32_t spi_id )
{
  ...
  UVOS_SPI_SetClockSpeed( UVOS_SDCARD_SPI, UVOS_SPI_PRESCALER_256 );
 ...
 */
int32_t UVOS_SDCARD_PowerOn( void )
{
  int32_t status;
  int i;
  ...
  UVOS_SPI_RC_PinSet( UVOS_SDCARD_SPI, 0, 1 ); /* spi, pin_value */
  ...
  UVOS_SPI_RC_PinSet( UVOS_SDCARD_SPI, 0, 0 ); /* spi, pin_value */

  /* wait for 1 mS */
  UVOS_DELAY_WaituS( 1000 );

  /* Send CMD0 to reset the media */
  if ( ( status = UVOS_SDCARD_SendSDCCmd( SDCMD_GO_IDLE_STATE, 0, SDCMD_GO_IDLE_STATE_CRC ) ) < 0 ) {
    goto error;
  }
  ...
  if ( ( status = UVOS_SDCARD_SendSDCCmd( SDCMD_SEND_IF_COND, 0x1AA, SDCMD_SEND_IF_COND_CRC ) ) == 0x01aa ) {
    /* SDHC Card Detected. */

    /* We now check to see if we should use block mode or byte mode. Command is SEND_OP_COND_SDC (ACMD41) with HCS (bit 30) set */
    for ( i = 0; i < 16384; i++ ) {
      if ( ( status = UVOS_SDCARD_SendSDCCmd( SDCMD_SEND_OP_COND_SDC, 0x01 << 30, SDCMD_SEND_OP_COND_SDC_CRC ) ) == 0 ) {
        break;
      }
    }

    if ( i < 16384 ) {
      status   = UVOS_SDCARD_SendSDCCmd( SDCMD_READ_OCR, 0, SDCMD_READ_OCR_CRC );
    ...
    if ( ( status = UVOS_SDCARD_SendSDCCmd( SDCMD_SEND_OP_COND_SDC, 0, SDCMD_SEND_OP_COND_SDC_CRC ) ) <= 1 ) {
      CardType = CT_SD1;
    } else {
      CardType = CT_MMC;
    }
    ...
    for ( i = 0; i < 16384; i++ ) {
      if ( CardType == CT_SD1 ) {
        status = UVOS_SDCARD_SendSDCCmd( SDCMD_SEND_OP_COND_SDC, 0, SDCMD_SEND_OP_COND_SDC_CRC );
      } else {
        status = UVOS_SDCARD_SendSDCCmd( SDCMD_SEND_OP_COND, 0, SDCMD_SEND_OP_COND_CRC );
      }
    ...
    UVOS_SDCARD_SendSDCCmd( SDCMD_SET_BLOCKLEN, 512, SDCMD_SEND_OP_COND_CRC );
  }
 ...
 * \return < 0 on errors
 */
int32_t UVOS_SDCARD_PowerOff( void )
{
  return 0; // no error
}
 ...
 *   sdcard_available = UVOS_SDCARD_CheckAvailable(prev_sdcard_available);
 *
 ...
 */
int32_t UVOS_SDCARD_CheckAvailable( uint8_t was_available )
{
  int32_t ret;
  ...
  if ( was_available ) {
    ...
    UVOS_SPI_SetClockSpeed( UVOS_SDCARD_SPI, UVOS_SPI_PRESCALER_4 );

    /* Activate chip select */
    UVOS_SPI_RC_PinSet( UVOS_SDCARD_SPI, 0, 0 ); /* spi, pin_value */

    /* Send STATUS command to check if media is available */
    ret = UVOS_SDCARD_SendSDCCmd( SDCMD_SEND_STATUS, 0, SDCMD_SEND_STATUS_CRC );
  } else {
    ...
    /* Wait a littl bit */
    UVOS_DELAY_WaituS( 50 );

    /* Activate chip select */
    UVOS_SPI_RC_PinSet( UVOS_SDCARD_SPI, 0, 0 ); /* spi, pin_value */

    /* Send CMD0 to reset the media */
    if ( ( ret = ( UVOS_SDCARD_SendSDCCmd( SDCMD_GO_IDLE_STATE, 0, SDCMD_GO_IDLE_STATE_CRC ) ) ) < 0 ) {
      goto not_available;
    }
 ...
 */
int32_t UVOS_SDCARD_SendSDCCmd( uint8_t cmd, uint32_t addr, uint8_t crc )
{
  int i;
  int32_t ret;
  ...
  if ( cmd & 0x80 ) { /* ACMD<n> is the command sequence of CMD55-CMD<n> */
    cmd &= 0x7F;
    ret  = UVOS_SDCARD_SendSDCCmd( SDCMD_APP_CMD, 0, SDCMD_APP_CMD_CRC );
    if ( ret > 1 ) {
      return ret;
    }
  }
  ...
  /* Transfer to card */
  UVOS_SPI_TransferByte( UVOS_SDCARD_SPI, ( uint8_t )cmd );
  UVOS_SPI_TransferByte( UVOS_SDCARD_SPI, ( addr >> 24 ) & 0xff );
  UVOS_SPI_TransferByte( UVOS_SDCARD_SPI, ( addr >> 16 ) & 0xff );
  UVOS_SPI_TransferByte( UVOS_SDCARD_SPI, ( addr >> 8 ) & 0xff );
  UVOS_SPI_TransferByte( UVOS_SDCARD_SPI, ( addr >> 0 ) & 0xff );
  UVOS_SPI_TransferByte( UVOS_SDCARD_SPI, crc );
  ...
  if ( cmd == SDCMD_SEND_STATUS ) {
    ...
    ret = UVOS_SPI_TransferByte( UVOS_SDCARD_SPI, 0xff );
    ret = UVOS_SPI_TransferByte( UVOS_SDCARD_SPI, 0xff );
    ...
    for ( i = 0; i < 8; ++i ) {
      if ( !( ( ret = UVOS_SPI_TransferByte( UVOS_SDCARD_SPI, 0xff ) ) & 0x80 ) ) {
        break;
      }
    }
    if ( i == 8 ) {
  ...
  if ( ( cmd == SDCMD_SEND_IF_COND || cmd == SDCMD_READ_OCR ) && timeout != 1 ) {
    ...
    ret  = ( UVOS_SPI_TransferByte( UVOS_SDCARD_SPI, 0xff ) << 24 );
    ret |= ( UVOS_SPI_TransferByte( UVOS_SDCARD_SPI, 0xff ) << 16 );
    ret |= ( UVOS_SPI_TransferByte( UVOS_SDCARD_SPI, 0xff ) << 8 );
    ret |= ( UVOS_SPI_TransferByte( UVOS_SDCARD_SPI, 0xff ) << 0 );
  }
 ...
 */
int32_t UVOS_SDCARD_SectorRead( uint32_t sector, uint8_t *buffer )
{
  int32_t status;
  int i;
  ...
  if ( ( status = UVOS_SDCARD_SendSDCCmd( SDCMD_READ_SINGLE_BLOCK, sector, SDCMD_READ_SINGLE_BLOCK_CRC ) ) ) {
  ...
  for ( i = 0; i < 65536; ++i ) { // TODO: check if sufficient
    uint8_t ret = UVOS_SPI_TransferByte( UVOS_SDCARD_SPI, 0xff );
    if ( ret != 0xff ) {
      break;
    }
  }
  ...
  UVOS_SPI_TransferBlock( UVOS_SDCARD_SPI, NULL, buffer, 512, NULL );
 ...
 */
int32_t UVOS_SDCARD_SectorWrite( uint32_t sector, uint8_t *buffer )
{
  int32_t status;
  int i;
  ...
  if ( ( status = UVOS_SDCARD_SendSDCCmd( SDCMD_WRITE_SINGLE_BLOCK, sector, SDCMD_WRITE_SINGLE_BLOCK_CRC ) ) ) {
    status = ( status < 0 ) ? -256 : status; /* Return timeout indicator or error flags */
    goto error;
  }

  /* Send start token */
  UVOS_SPI_TransferByte( UVOS_SDCARD_SPI, 0xfe );

  /* Send 512 bytes of data via DMA */
  UVOS_SPI_TransferBlock( UVOS_SDCARD_SPI, buffer, NULL, 512, NULL );
  ...
  /* Read response */
  uint8_t response = UVOS_SPI_TransferByte( UVOS_SDCARD_SPI, 0xff );
 ...
 * \return -257 if timeout while waiting for start token
 */
int32_t UVOS_SDCARD_CIDRead( SDCARDCidTypeDef *cid )
{
  int32_t status;
  int i;
  ...
  if ( ( status = UVOS_SDCARD_SendSDCCmd( SDCMD_SEND_CID, 0, SDCMD_SEND_CID_CRC ) ) ) {
    status = ( status < 0 ) ? -256 : status; /* return timeout indicator or error flags */
    goto error;
  }
  ...
  UVOS_SPI_TransferBlock( UVOS_SDCARD_SPI, NULL, cid_buffer, 16, NULL );
 ...
 * \return -257 if timeout while waiting for start token
 */
int32_t UVOS_SDCARD_CSDRead( SDCARDCsdTypeDef *csd )
{
  int32_t status;
  int i;
  ...
  if ( ( status = UVOS_SDCARD_SendSDCCmd( SDCMD_SEND_CSD, 0, SDCMD_SEND_CSD_CRC ) ) ) {
    status = ( status < 0 ) ? -256 : status; /* Return timeout indicator or error flags */
    goto error;
  }
  // wait for start token of the data block
  ...
  UVOS_SPI_TransferBlock( UVOS_SDCARD_SPI, NULL, csd_buffer, 16, NULL );
 ...
 */
int32_t UVOS_SDCARD_StartupLog( void )
{
  FILEINFO File;
  char Buffer[1024];
  uint32_t Cache;
  ...
  DFS_UnlinkFile( &UVOS_SDCARD_VolInfo, ( uint8_t * )LOG_FILENAME, UVOS_SDCARD_Sector );

  if ( DFS_OpenFile( &UVOS_SDCARD_VolInfo, ( uint8_t * )LOG_FILENAME, DFS_WRITE, UVOS_SDCARD_Sector, &File ) ) {
    /* Error opening file */
    return -2;
  }

  sprintf( Buffer, "UVOS Startup Log\r\n\r\nLog file creation completed.\r\n" );
  if ( DFS_WriteFile( &File, UVOS_SDCARD_Sector, ( uint8_t * )Buffer, &Cache, strlen( Buffer ) ) ) {
  ...
  /* sprintf(Buffer, "Total Space: %u MB\r\nFree Space: %u MB\r\n", (uint16_t)((VolInfo.numclusters * (VolInfo.secperclus / 2)) / 1024), (uint16_t)(UVOS_SDCARD_GetFree() / 1048576)); */

  sprintf( Buffer, "Total Space: %u MB\r\n\r\n", ( uint16_t )( ( UVOS_SDCARD_VolInfo.numclusters * ( UVOS_SDCARD_VolInfo.secperclus / 2 ) ) / 1024 ) );
  if ( DFS_WriteFile( &File, UVOS_SDCARD_Sector, ( uint8_t * )Buffer, &Cache, strlen( Buffer ) ) ) {
    /* Error writing to file */
    return -2;
  }
 ...
 */
int32_t UVOS_SDCARD_MountFS( uint32_t CreateStartupLog )
{
  uint32_t pstart, psize;
  uint8_t pactive, ptype;
  uint8_t sdcard_available = 0;

  sdcard_available = UVOS_SDCARD_CheckAvailable( 0 );
  ...
  pstart = DFS_GetPtnStart( 0, UVOS_SDCARD_Sector, 0, &pactive, &ptype, &psize );
  if ( pstart == 0xffffffff ) {
    /* Cannot find first partition */
    return -2;
  }

  if ( DFS_GetVolInfo( 0, UVOS_SDCARD_Sector, pstart, &UVOS_SDCARD_VolInfo ) != DFS_OK ) {
    /* No volume information */
    return -3;
  }

  if ( CreateStartupLog == 1 ) {
    if ( UVOS_SDCARD_StartupLog() ) {
 ...
 */
int32_t UVOS_SDCARD_GetFree( void )
{
  ...
  for ( uint32_t i = 2; i < UVOS_SDCARD_VolInfo.numclusters; ++i ) {
    if ( !DFS_GetFAT( &UVOS_SDCARD_VolInfo, UVOS_SDCARD_Sector, &ScratchCache, i ) ) {
      VolFreeBytes += UVOS_SDCARD_VolInfo.secperclus * SECTOR_SIZE;
    }
  }
 ...
 */
int32_t UVOS_SDCARD_ReadBuffer( PFILEINFO fileinfo, uint8_t *buffer, uint32_t len )
{
  uint32_t SuccessCount;

  if ( DFS_ReadFile( fileinfo, UVOS_SDCARD_Sector, buffer, &SuccessCount, len ) ) {
    /* DFS_ReadFile failed */
    return -1;
  }
  if ( SuccessCount != len ) {
 ...
 */
int32_t UVOS_SDCARD_ReadLine( PFILEINFO fileinfo, uint8_t *buffer, uint32_t max_len )
{
  int32_t status;
  uint32_t num_read = 0;

  while ( fileinfo->pointer < fileinfo->filelen ) {
    status = UVOS_SDCARD_ReadBuffer( fileinfo, buffer, 1 );
 ...
 */
int32_t UVOS_SDCARD_FileCopy( char *Source, char *Destination )
{
  ...
  if ( DFS_OpenFile( &UVOS_SDCARD_VolInfo, ( uint8_t * )Source, DFS_READ, UVOS_SDCARD_Sector, &SourceFile ) ) {
    /* Source file doesn't exist */
    return -1;
  } else {
    /* Delete destination file if it already exists - ignore errors */
    DFS_UnlinkFile( &UVOS_SDCARD_VolInfo, ( uint8_t * )Destination, UVOS_SDCARD_Sector );

    if ( DFS_OpenFile( &UVOS_SDCARD_VolInfo, ( uint8_t * )Destination, DFS_WRITE, UVOS_SDCARD_Sector, &DestFile ) ) {
  ...
  do {
    if ( DFS_ReadFile( &SourceFile, UVOS_SDCARD_Sector, WriteBuffer, &SuccessCountRead, SECTOR_SIZE ) ) {
      /* DFS_ReadFile failed */
      return -3;
    } else if ( DFS_WriteFile( &DestFile, UVOS_SDCARD_Sector, WriteBuffer, &SuccessCountWrite, SuccessCountRead ) ) {
 ...
 * return -1 Error deleting file
 */
int32_t UVOS_SDCARD_FileDelete( char *Filename )
{
  if ( DFS_UnlinkFile( &UVOS_SDCARD_VolInfo, ( uint8_t * )Filename, UVOS_SDCARD_Sector ) ) {
    /* Error deleting file */
    return -1;
  }
